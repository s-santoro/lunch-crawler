...
class Preprocessor(Task):
    ...
    def output(self):
        ...
        return LocalTarget("./logs/%s_Preprocessor_out.csv" % (prefix), format=UTF8)
    ...
    def run(self):
        ...
        output_df = pd.DataFrame(columns=('text', 'cleaned_text', 'url', 'title', 'filename'))
        ...
        for index, document in df.iterrows():
            ...
            row = [document.text, text, document.url, title, document.filename]
    ...
    def beverageTagger(self, text):
        bev = pd.read_csv('./helper_files/beverage_list.txt', header=None)
        bev.columns = ['word']
    ...
    def removeStopWords(self, text):
        ...
        stop = pd.read_csv('./helper_files/stopwords_no_umlaute.txt', header=None)
        stop.columns = ['word']
